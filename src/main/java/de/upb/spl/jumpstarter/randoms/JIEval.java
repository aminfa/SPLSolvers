...
import de.upb.spl.finish.SingleObjScore;
...
public class JIEval {
    ...
    public void eval() {
        base = new InlineBaseInterpreter();

        base = new ConfiguredEnv(base);
        base.configuration().setProperty("de.upb.spl.SPLReasoner.evaluations", "100");
        base.configuration().setProperty("de.upb.spl.eval.hvTimeline.samples", "100");
        ...
        benchmarks.values().forEach(benchmark -> benchmark.replays.forEach(replayer -> {
            ...
            for (int i = 0; i < evaluationSamples.size(); i++) {
                ...
                SingleObjScore calculator = new SingleObjScore(measurements, sampleIndex, min[0], max[0]);
                calculator.run();
